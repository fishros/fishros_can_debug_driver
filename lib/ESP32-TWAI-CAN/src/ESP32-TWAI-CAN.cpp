...
TwaiSpeed TwaiCAN::convertSpeed(uint16_t canSpeed)
{
    ...
    case 1:
        actualSpeed = TWAI_SPEED_1KBPS;
        break;
    case 5:
        actualSpeed = TWAI_SPEED_5KBPS;
        break;
    case 10:
        actualSpeed = TWAI_SPEED_10KBPS;
        break;
#endif
#if (SOC_TWAI_BRP_MAX > 128) || (CONFIG_ESP32_REV_MIN_FULL >= 200)
    case 13: // Just to handle those who would round upwards..
    case 12:
        actualSpeed = TWAI_SPEED_12_5KBPS;
        break;
    case 16:
        actualSpeed = TWAI_SPEED_16KBPS;
        break;
    case 20:
        actualSpeed = TWAI_SPEED_20KBPS;
        break;
#endif
    case 50:
        actualSpeed = TWAI_SPEED_50KBPS;
        break;
    case 100:
        actualSpeed = TWAI_SPEED_100KBPS;
        break;
    case 125:
        actualSpeed = TWAI_SPEED_125KBPS;
        break;
    case 250:
        actualSpeed = TWAI_SPEED_250KBPS;
        break;
    case 500:
        actualSpeed = TWAI_SPEED_500KBPS;
        break;
    case 800:
        actualSpeed = TWAI_SPEED_800KBPS;
        break;
    case 1000:
        actualSpeed = TWAI_SPEED_1000KBPS;
        break;
    }
    return actualSpeed;
}
                    ...
                    twai_timing_config_t *tConfig)
{
    ...
    if (end())
    {
        ...
        twai_general_config_t g_config = {.mode = TWAI_MODE_NORMAL, .tx_io = (gpio_num_t)tx, .rx_io = (gpio_num_t)rx, .clkout_io = TWAI_IO_UNUSED, .bus_off_io = TWAI_IO_UNUSED, .tx_queue_len = txQueueSize, .rx_queue_len = rxQueueSize, .alerts_enabled = TWAI_ALERT_NONE, .clkout_divider = 0, .intr_flags = ESP_INTR_FLAG_LEVEL1};
        ...
        // f_config.acceptance_mask = 0x00000001;

        if (!gConfig)
            gConfig = &g_config;
        if (!tConfig)
            tConfig = &t_config[speed];
        if (!fConfig)
            fConfig = &f_config;
    ...
    if (readStatus(&status))
    {
        if (status.state == TWAI_STATE_BUS_OFF)
        {
            if (twai_driver_uninstall() == ESP_OK)
            {
                ...
                // Install TWAI driver
                if (twai_driver_install(&g_config, &t_config[speed], &f_config) == ESP_OK)
                {
                    LOG_TWAI("Driver installed");
                    // Start TWAI driver
                    if (twai_start() == ESP_OK)
